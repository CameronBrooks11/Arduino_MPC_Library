...
![Prediction formulation](docs/Formulation_of_Prediction_Variables.png "Click to maximize if the image rescaling make you dizzy")

Remark: 
1. `HP` is the Prediction Horizon (<img src="docs/eq_render_uncon/hp_is_pos.gif" align="middle"/>). This constant determine how far ahead we'll predict the system's evolution.
2. `Hu` is the Control Horizon (<img src="docs/eq_render_uncon/hu_between_hp_and_pos.gif" align="middle"/>). This constant determine how far ahead we'll calculate the control action.
3. As you can see on the last equation, the variable we'll calculate is <img src="docs/eq_render_uncon/duk_hat_to_du_hp_hat.gif" align="top"/>. But actually we only need <img src="docs/eq_render_uncon/duk_hat.gif" align="top"/> to calculate the control action <img src="docs/eq_render_uncon/uk.gif" align="top"/>. So we'll calculate all control prediction, extract <img src="docs/eq_render_uncon/duk_hat.gif" align="top"/>, and discard the rest.
...
3. `R` constant is the penalty for big <img src="docs/eq_render_uncon/du_hat.gif" align="bottom"/>, because sometimes we don't want <img src="docs/eq_render_uncon/uk.gif" align="bottom"/> to change as much as possible (for example, if our controller have gear inside, we don't want the control action to change rapidly to reduce wear).
4. We need the `Q` and `R` matrices to be positive-definite to ensure the optimal control formulation to be convex and have one global optimum (and also to ensure <img src="docs/eq_render_uncon/h=cthetatrQctheta+r.gif" align="top"/> as nonzero and invertible). The simplest way to ensure that condition is to make `Q` and `R` matrices as diagonal matrix and set the diagonal entries with positive value.
...
1. The `H` matrix and (some of calculation inside) `G` matrix (specifically the <img src="docs/eq_render_uncon/2cthetatrQ.gif"  align="top"/> portion) are actually constant.
2. The equation <img src="docs/eq_render_uncon/duk=0.5hinvg.gif" align="middle"/> can be described as <img src="docs/eq_render_uncon/duk=0.5hinv2cthetatrqQek=hinvcthetatrqQek.gif" align="middle"/>. And actually we don't need all row of the (constant) matrix <img src="docs/eq_render_uncon/hinvcthetatrqQ.gif" align="top"/> (because we only need the first M-th row to calculate <img src="docs/eq_render_uncon/duk.gif" align="top"/>).
...
2. The <img src="docs/eq_render_uncon/thetac.gif" align="bottom"/> matrix is often ill conditioned, so the <img src="docs/eq_render_uncon/cthetatrQcheta.gif" align="top"/> calculation is bad.
